package com.example.andriesbruno.instructionsshow;
...
public class MapsActivity extends AppCompatActivity implements
    ...
    private String str = "kybxE_vosWMuB@]V{A`@yB`@iBr@}B\\}@HEJa@O[UAIDIVIb@g@|Ak@rBi@lCu@tDy@fBs@B";
    ...
    //crate latlng data for testing
    double start_point_y = 129.191513;
    double start_point_x = 35.572904;
    ...
    public void draw_polyline() {
        LatLng start = new LatLng(start_point_x, start_point_y);
        for (int i = 0; i < pointY.length; i++) {
            mMap.addPolyline(new PolylineOptions()
                    .add(start, new LatLng(pointX[i], pointY[i]))
                    .width(5)
                    .color(Color.RED)
                    .geodesic(true)
            );

            start = new LatLng(pointX[i], pointY[i]);
        }
    }
    ...
    public void  test(){
        ...
        options.addAll(decodedPath);
        mMap.addPolyline(options);
    }


    /**
     * This will be used to query the osrm server directly
     */
    ...
    /**
     * This function query's the osrm server for matched location points
     */
